...
export default function Table() {
    ...
    return (
        <div className="bg-gradient-to-r bg-[#00000000] text-white rounded-xl shadow-xl xl:h-[100vh] w-[100vw] flex flex-col gap-5 justify-start items-center" style={{ fontFamily: 'Montserrat, sans-serif' }}>
            {/* Filters */}
            <Navbar />
            <div className="bg-[#00000048] xl:h-[40vh] xl: w-[90vw] rounded-3xl flex justify-center items-center m-5" >
                <form onSubmit={handleSubmit} className="w-full">
                    ...
                    <div className="text-white xl:space-y-4 font-bold text-xl xl:w-full">
                        ...
                        >
                            ...
                            {[10, 20, 30, 50, 100].map(size => (
                                <option key={size} value={size} className="bg-[#222222]">{size}</option>
                            ))}
                        </select>
                    ...
                    <button type="submit" className="mt-6 px-6 py-2 rounded-lg bg-black hover:bg-gray-800 text-white font-bold w-[90%] mb-3 xl:w-[50%] text-xl shadow-md mb-2">
                        {loading ? (
                            <Typewriter
                                words={[
                                    'Matching filters...',
                    ...
                    <div className="m-2 text-center text-xl font-bold">
                        {loading && <span className="text-red-500">Time taken: {formatTime(elapsed)}</span>}
                        {success && !loading && <span className="text-white">Loaded in {formatTime(elapsed)}</span>}
                    </div>
                </form>
            </div>
